...
function [EEG, com] = bv_load(path, hdrfile, flag, srange, chans)

com = '';
EEG = [];
if nargin < 3 flag = 1; end
if nargin < 2
    [hdrfile path] = uigetfile('*.vhdr', 'Select Brain Vision vhdr-file - bv_load()');
    ...
    uilist = {{ 'style' 'text' 'string' 'Interval (samples; e.g., [7 42]; default: all):'} ...
              ...
              { 'style' 'edit' 'string' ''}};
    result = inputgui(uigeom, uilist, 'pophelp(''bv_load'')', 'Load a Brain Vision Data Exchange format dataset');
...
if isfield(hdr.commoninfos, 'markerfile')
    ...
    if hdr.commoninfos.datafile ~= MRK.commoninfos.datafile
        disp('bv_load() warning: data files in header and marker files inconsistent.');
    end
...
if nargout == 2
    com = sprintf('EEG = bv_load(''%s'', ''%s'', %s, %s);', path, hdrfile, mat2str(srange), mat2str(chans));
end
